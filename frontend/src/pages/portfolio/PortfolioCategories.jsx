...
function PortfolioCategories() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await getPortfolioCategoriesApi();
  ...
  const handleSearch = async (e) => {
    ...
    try {
      if (query.trim()) {
        const response = await searchPortfolioCategoriesApi(query);
        setCategories(response.categories || []);
      } else {
        fetchCategories();
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await updatePortfolioCategoryApi({ name: formData.name, status: 'ACTIVE' }, editId);
        toast.success('Category updated successfully');
      } else {
        await createPortfolioCategoryApi({ name: formData.name });
  ...
  const handleDelete = async () => {
    setIsSubmitting(true);
    try {
      await deletePortfolioCategoryApi(deleteModal.id);
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900">
            {isEditing ? 'Edit Portfolio Category' : 'Create Portfolio Category'}
          </h2>
          {isEditing && (
            <button
              onClick={resetForm}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="px-3 py-1 inline-flex text-xs font-medium rounded-full bg-primary-100 text-[#25b485]">
                          {category.projectsCount || 0} projects
                        </span>
                      </td>
